...
export function Logo({ href = "/" }: { href?: string }) {
  return (
    <Link href={href} className="flex items-center gap-3" aria-label="TripGlass home">
...
export function MarketingNavbar({ isSignedIn = false }: { isSignedIn?: boolean }) {
  return (
    <header className="fixed left-0 right-0 top-0 z-40 px-4 py-4 sm:px-8">
      <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-3xl border border-white/[0.15] bg-white/[0.07] px-4 py-3 backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-2">
          <Button href={isSignedIn ? "/dashboard" : "/login"} variant="ghost" className="hidden sm:inline-flex">
