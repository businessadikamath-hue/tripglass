...
function systemPrompt() {
  return [
    ...
    "Use provided Duffel travel offers for live flight and hotel pricing when available, but always say prices can change before booking.",
    ...
    const repairedText = await requestGeminiJson(
      JSON.stringify({
        ...
        hard_requirements: [
          ...
          "Each place.source must be google_places, duffel, ai_estimate, or user_input.",
        ],
      }),
    );
...
export async function generateItineraryWithGemini(args: {
  ...
  return generateStructuredItinerary(
    JSON.stringify({
      ...
      live_travel_offers: args.travelOffers ?? null,
